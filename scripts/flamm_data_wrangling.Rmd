...
write_csv(flam.df, here('data', 'processed-data', 'all_flamm_data.csv'))
```

# Saving .csv
```{r}
flam.df.final <- flam.df %>% 
  filter(!is.na(heat_flux_change)) # removing outlier in heat flux change, and removing all nonignitions

summary(flam.df.final)

write_csv(flam.df.final, here('data', 'processed-data', 'clean_flamm_data.csv'))
```
