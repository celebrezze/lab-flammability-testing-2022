...
tukey.labels.df <- rbind(fh.spp.tukey.labels, fd.spp.tukey.labels, tc.spp.tukey.labels, hf.spp.tukey.labels, sw.spp.tukey.labels, branching.spp.tukey.labels, no.tukey.labels.df)
```
...
flamm.df %>% 
  ...
  pivot_longer(cols = c(fh, fd, temp_change, heat_flux_change, sample_wt,
                        branching, lfm, dmc, leaf_mass_ratio),
               names_to = 'variable',
               values_to = 'value') %>% 
  mutate(species = fct_relevel(species, c('HETARB', 'MALLAU', 'CEAGRI', 'SALAPI', 'SALLEU', 'ARCDEN', 'ARTCAL', 'ERIKAR'))) %>% 
  ggplot() +
