...
```{r}
#expfolder = '/Users/kristinafauss/BackupToBox/Git/lab-flammability-testing-2022/model_summary_tables'
#write.csv(main.ig.glmm.df, "/Users/kristinafauss/BackupToBox/Git/lab-flammability-testing-2022/model_summary_tables/mainIGmem", row.names=FALSE)
```
...
```{r}
# WITH REML = TRUE, to report coefficients
# top models identified in model selection for each flammability metric
fh.top.mem <- lmer(fh ~ sample_wt + mpa + branching + start_temp + stem_sav + (1|species/plant_id), REML = T, data = mem.df)
fd.top.mem <- lmer(fd ~ sample_wt*lfm + leaf_mass_ratio + dmc + branch_volume + (1|species/plant_id), REML = T, data = mem.df)
temp.top.mem <- lmer(temp_change ~ start_temp + sample_wt + branching + lfm + (1|species/plant_id), REML = T, data = mem.df)
hf.top.mem <- lmer(heat_flux_change ~ sample_wt*start_temp + LMA + (1|species/plant_id), REML = T, data = mem.df)

coef.top.mem.list <- list(fh.top.mem, fd.top.mem, temp.top.mem, hf.top.mem) # list of models

for(i in 1:length(coef.top.mem.list)){
  print(multicollinearity(coef.top.mem.list[[i]]))
} # double check multicollinearity

# sjPlot tab_model to make a summary table of the top mixed effects models
...
```{r}
# selected predictors, as they are coded
pred_vec <- c('branch_volume', 'branching', 'dmc', 'leaf_mass_ratio', 'lfm', 'LMA', 'mpa', 'sample_wt', 'sample_wt:lfm', 'sample_wt:start_temp', 'start_temp', 'stem_sav')
predictors <- rep(pred_vec, 4)
flam_metrics <- c(rep('Flame\nHeight', length(pred_vec)),
                  rep('Flame\nDuration', length(pred_vec)),
                  rep('Temperature\nChange', length(pred_vec)),
                  rep('Heat Flux\nChange', length(pred_vec)))
# coefficients, in the order of the above pred_vec
coef <- c(NA, 0.174, NA, NA, NA, NA, 0.109,
          0.475, NA, NA, 0.137, -0.319, # fh
          -0.144, NA, 0.377, 0.186, -0.168, NA, NA,
          0.489, -0.393, NA, NA, NA, # fd
          NA, 0.193, NA, NA, -0.167, NA, NA,
          0.192, NA, NA, -0.511, NA, # temp
          NA, NA, NA, NA, NA, -0.014, NA,
          0.657, NA, -0.228, 0.241, NA) # hf
# p values, in the order of the coef and pred_vec
p.val <- c(NA, '**', NA, NA, NA, NA, NA,
          '***', NA, NA, NA, '*', # fh
          NA, NA, '*', NA, NA, NA, NA,
          '***', '*', NA, NA, NA, # fd
          NA, '*', NA, NA, NA, NA, NA,
          ...
          '***', NA, '**', '**', NA) # hf
mem.visualization.df <- data.frame(predictors, flam_metrics, coef, p.val) %>% 
  mutate(predictors = case_when( # naming the pred_vec for visualization
...
ggplot(aes(x = predictors, y = flam_metrics, fill = coef)) +
  ...
  labs(x = 'Predictors', y = 'Flam. Metrics',
       fill = '\u03b2 Coefficient') +
  scale_fill_gradient2(low = 'darkred', mid = "gray80", high = 'darkblue', midpoint = 0, na.value = 'white', limits = c(-0.66, 0.66), breaks = c(-0.6, -0.3, 0, 0.3, 0.6)) +
