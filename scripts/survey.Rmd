...
# Raw Data
```{r, warning = FALSE}
data <- read_csv(here("data", "raw-data", "survey", "survey_data.csv"), show_col_types = FALSE) %>% 
  clean_names() %>% 
...
```{r}
pie.text <- pct_plants %>% 
  arrange(desc(trait_names)) %>% 
  mutate(prop = pct / sum(pct_plants$pct) *100) %>%
  mutate(ypos = cumsum(prop)- 0.5*prop) %>% 
  mutate(trait_names = ifelse(trait_names == 'Wildlife Habitat', 'Wildlife \nHabitat', trait_names))
pie.text$rot <- c(70, 47, 20, -25, 0)

desirable.pie <- pct_plants %>%
  ggplot(aes(fill = trait_names, y = pct, x = '')) +
  geom_bar(stat = 'identity', width = 1) +
  coord_polar('y', start = 0) +
  labs(y = '% of Responses',
       fill = "Trait") +
  geom_text(data = pie.text, aes(y = ypos, label = trait_names, angle = rot), color = "white", size=4.5, face = 'bold') +
  theme_void() +
  theme(legend.position = 'none')
desirable.pie
```
...
## Doughnut Plot
```{r}
doughnut_pct <- pct_plants %>%
  mutate(pct = pct/100) %>% 
  arrange(desc(pct))

# Compute the top of each rectangle
doughnut_pct$ymax = cumsum(doughnut_pct$pct)

# Compute the bottom of each rectangle
doughnut_pct$ymin = c(0, head(doughnut_pct$ymax, n=-1))

# Compute label position
doughnut_pct$labelPosition <- (doughnut_pct$ymax + doughnut_pct$ymin) / 2

desirable.doughnut <- doughnut_pct %>% 
  ggplot(aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = trait_names)) +
  geom_rect() +
  #geom_label(x = 3.5, aes(y = labelPosition, label = trait_names), size = 4) +
  scale_fill_manual(values = c('#ADB6B6B2', '#42B540B2', '#1B1919B2', '#925E9FB2', '#0099B4B2')) +
  coord_polar(theta = 'y') +
  labs(y = '% of Responses',
       fill = "Trait") +
  xlim(2.5, 4) +
  ...
  theme(legend.position = 'none')
desirable.doughnut
ggsave(here("figures", 'main_figures', 'desirable_doughnut.jpg'), height = 8, width = 8) 
```
...
melted <- reshape::melt(es_likert_df) %>% 
  ...
  mutate(item_title = str_replace_all(item_title, "For", "for")) %>% 
  mutate(item_title = str_replace_all(item_title, "The", "the")) %>% 
  mutate(item_title = str_replace_all(item_title, "My", "my")) %>% 
  mutate(item_title = str_replace_all(item_title, "From", "from")) %>% 
  mutate(item_title = str_replace_all(item_title, "Food/Home Garden", "Home Garden")) %>% 
  mutate(item_title = str_replace_all(item_title, "Cooling Provides Shade",
                                      "Cooling/Shade")) %>% 
  mutate(item_title = str_replace_all(item_title, "Protect/Shield my Home from Wildfire",
                                      "Wildfire Protection")) %>% 
  mutate(item_title = str_replace_all(item_title, "Food/Habitat for Wild Animals", 
                                      "Wildlife")) %>% 
  mutate(item_title = str_replace_all(item_title, "Place for Socialization",
                                      "Socializing")) %>% 
  mutate(item_title = str_replace_all(item_title, "Observing the Natural World",
                                      "Observing Nature")) %>% 
  mutate(item_title = str_replace_all(item_title, "Soil Fertility Control",
                                      "Soil Fertility")) %>% 
  mutate(item_title = str_replace_all(item_title, "Commercial Agriculture",
                                      "Agriculture")) %>% 
...
max_value_very_high <- melted %>% 
  ...
  group_by(item_title) %>% 
  summarise(very_high_value = max(value)) %>% 
  select(item_title, very_high_value)
...
cultural <-  melted %>% 
  ...
  theme(legend.position="none", 
        axis.title.y = element_text(face = 'bold', size = 22, vjust = 3.3, hjust = 0.3),
        axis.text.y = element_text(size = 18),
        axis.text.x = element_blank(),
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank(),
        #plot.title = element_text(face = 'bold', size = 16),
        plot.margin = unit(c(.25, 0, .1, 0.5), "cm")
        )
cultural
...
reg <-  melted %>% 
  ...
  #scale_fill_brewer(palette="BrBG")+
  theme(legend.position="none", 
        axis.title.y = element_text(face = 'bold', size = 22, vjust = 3.3),
        axis.text.y = element_text(size = 18),
        axis.text.x = element_blank(),
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank(),
        #plot.title = element_text(face = 'bold', size = 16),
        plot.margin = unit(c(0, 0, .1, .5), "cm")
        )
reg
