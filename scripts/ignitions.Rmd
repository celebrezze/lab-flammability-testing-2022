...
#  select(lfm, LMA, sample_wt, leaf_mass_ratio, branching, mpa, start_temp, dmc, branch_volume, stem_sav, leaf_sav, thickness) # species

# primary predictors
mem.predictors.df <- mem.df %>% 
  select(lfm, LMA, sample_wt, leaf_mass_ratio, branching, start_temp, stem_sav, leaf_sav, branch_volume, dmc)

# Second selection (see below note)
#mem.predictors.df <- mem.df %>% 
#  select(lfm, sample_wt, mpa, branching, dmc, branch_volume, sample_density, leaf_area, dw_flam_sample, ww_flam_sample, thickness)
...
predictors <- c('lfm', 'LMA', 'sample_wt', 'leaf_mass_ratio', 'branching', 'dw_flam_sample', 'start_temp', 'ww_flam_sample', 'branch_volume', 'dmc', 'mpa', 'thickness', 'stem_sav', 'leaf_sav')

glmm.ig.int.selection <- glmm.int.selection('ignition_binary', predictors, mem.df, binomial, zero.inflation = '0', rem.str = '(1|species/plant_id)')
...
### Adding Predictors
```{r}
ig.int.glmm.list[[56]] <- glmmTMB(ignition_binary ~ start_temp*thickness + branching + (1|species/plant_id), family = binomial, data = mem.df)
AIC(ig.int.glmm.list[[56]])

ig.int.glmm.list[[57]] <- glmmTMB(ignition_binary ~ start_temp*thickness + branch_volume + (1|species/plant_id), family = binomial, data = mem.df)
AIC(ig.int.glmm.list[[57]])

ig.int.glmm.list[[58]] <- glmmTMB(ignition_binary ~ start_temp*thickness + lfm + (1|species/plant_id), family = binomial, data = mem.df)
AIC(ig.int.glmm.list[[58]])

ig.int.glmm.list[[59]] <- glmmTMB(ignition_binary ~ start_temp*thickness + stem_sav + (1|species/plant_id), family = binomial, data = mem.df)
AIC(ig.int.glmm.list[[59]]) # AIC = 192.35
r2_nakagawa(ig.int.glmm.list[[59]])
summary(ig.int.glmm.list[[59]])

ig.int.glmm.list[[60]] <- glmmTMB(ignition_binary ~ stem_sav*leaf_sav + branching + (1|species/plant_id), family = binomial, data = mem.df)
AIC(ig.int.glmm.list[[60]])

ig.int.glmm.list[[61]] <- glmmTMB(ignition_binary ~ stem_sav*leaf_sav + start_temp + (1|species/plant_id), family = binomial, data = mem.df)
AIC(ig.int.glmm.list[[61]])

ig.int.glmm.list[[62]] <- glmmTMB(ignition_binary ~ stem_sav*leaf_sav + branch_volume + (1|species/plant_id), family = binomial, data = mem.df)
AIC(ig.int.glmm.list[[62]])

ig.int.glmm.list[[63]] <- glmmTMB(ignition_binary ~ stem_sav*leaf_sav + lfm + (1|species/plant_id), family = binomial, data = mem.df)
AIC(ig.int.glmm.list[[63]])

ig.int.glmm.list[[64]] <- glmmTMB(ignition_binary ~ stem_sav*leaf_sav + thickness + (1|species/plant_id), family = binomial, data = mem.df)
...
```{r}
m1 <- glmmTMB(ignition_binary ~ species + branching + thickness + (1|plant_id), family = binomial, data = mem.df)
multicollinearity(m1)

multicollinearity(ig.int.glmm.list[[57]])
```
