...
# Visualization
# 'ARTCAL', 'ARCDEN', 'CEAGRI', 'ERIKAR', 'HETARB', 'MALLAU', 'SALAPI', 'SALLEU'
flam_scores$laby_updated <- c(-1.06, -0.62, 0.78, -1.32, -0.25, 0.03, 0.45, 1.8)
flam_scores$labx_updated <- c(3.1, 2.55, 2.59, 2.47, 3.15, 3.16, 3.2, 2.77)
flam_scores$species_updated <- c('A. cal.', 'A. den.', 'C. gri.', 'E. kar.', 'H. arb.', 'M. lau.', 'S. api.', 'S. leu.')

es.vs.flam <- ggplot(data=flam_scores) +
  geom_point(aes(x=es.score, y=flam.score, color=species), size=12, alpha=0.75) + 
  geom_text(vjust=0, hjust=-0.15, aes(x=labx_updated, y=laby_updated, label=species_updated), fontface = 'bold.italic', size = 12) +
...
```{r}
# linear regression
desireflam.lm <- lm(flam.score ~ es.score, data = flam_scores)
summary(desireflam.lm)

```


## Adding Defensible Space Zones
```{r}
es.vs.flam.zones <- ggplot(data=flam_scores) +
  geom_point(aes(x=es.score, y=flam.score, color=species), size=12, alpha=0.75) + 
  geom_text(vjust=0, hjust=-0.15, aes(x=labx_updated, y=laby_updated, label=species_updated), fontface = 'bold.italic', size = 12) +
  #annotate(geom = 'rect', xmin = 1, xmax = 4, ymin = -2, ymax = -1,
  #         alpha = 0.2, fill = '#00CC99') +
  #annotate('text', x = 1.1, y = -1.5, label = 'Zone 1', size = 9,
  #         fontface = 'bold', color = '#005641', angle = 90) +
  #annotate(geom = 'rect', xmin = 1, xmax = 4, ymin = -1, ymax = 1,
  #         alpha = 0.2, fill = '#FFEE88') +
  #annotate('text', x = 1.1, y = 0, label = 'Zone 2', size = 9,
   #        fontface = 'bold', color = '#AE9400', angle = 90) +
  #annotate(geom = 'rect', xmin = 1, xmax = 4, ymin = 1, ymax = 2,
  #         alpha = 0.2, fill = '#D7263D') +
  #annotate('text', x = 1.1, y = 1.5, label = 'Avoid', size = 9,
  #         fontface = 'bold', color = '#750010', angle = 90) +
  labs(x = 'Desirable Traits Score', y = 'Flammability Score') +
  scale_color_manual(values = c('#00468BB2', '#ED0000B2', '#42B540B2', '#0099B4B2', '#925E9FB2', '#AD002AB2', '#ADB6B6B2', '#1B1919B2')) +
  theme_bw() +
  scale_y_continuous(limits = c(-2, 2)) +
...
```{r}
plot_grid(es.vs.flam.zones, flam.triangles, labels = c('a', 'b'), nrow = 1, rel_widths = c(2.5, 2), label_size = 50, greedy = F)
