...
```{r}
flam.df <- read.csv(here('data', 'processed-data', 'all_flamm_data.csv')) %>% 
  select(-date)
...
# Removing IRIDOU
We decided to remove IRIDOU from consideration pretty late on, so it is removed here rather than earlier on in the data wrangling pipeline
```{r}
main_all_df <- main_df
...
# Writing csv
```{r}
write.csv(main_all_df, here('data', 'processed-data', 'all_data.csv'))
